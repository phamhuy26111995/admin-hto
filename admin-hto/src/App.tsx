...
function App() {
  ...
  const { userInfo, permissions } = useSelector((state: any) => state.global);
  ...
  console.log(MappingRoutes.get(permissions[0]))
  return (
    <ConfigProvider
      theme={{
        algorithm: displayTheme ? theme.darkAlgorithm : theme.defaultAlgorithm,
      }}
    >
      <AntApp>
        <BrowserRouter>
          <Routes>
            ...
            >
              <Route key={"home"} element={permissions.length > 0 ? MappingRoutes.get(permissions[0]).component : <NotFoundPage />} path={"/"} />
